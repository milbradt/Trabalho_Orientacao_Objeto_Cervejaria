<?php
    ...
    class Banco 
    {
        ...
        public function setUsuario($nome, $cpf, $email, $senha, $dataNasc, $admin)
        {
            $res = $this->con->prepare("INSERT INTO usuario (`nome`, `cpf`, `email`, `senha`, `dataNasc`, `admin`) VALUES (?,?,?,?,?,?)");
            ...
            $res->bindValue(6, $admin, PDO::PARAM_INT);
            if ($res->execute() == TRUE) {
                return true;
            } else {
                return false;
        ...
        {
            $res = $this->con->query("SELECT * FROM usuario WHERE cpf = $cpf");
            return $res->fetchAll();
        }
        ...
        public function updateUsuario($nome, $cpf, $senha, $email, $dataNasc, $admin, $id)
        {

            $res = $this->con->prepare("UPDATE usuario SET nome = ?, cpf = ?, senha = ?, email = ?, dataNasc = ?, admin = ? WHERE id = ?");

            $res->bindValue(1, $nome, PDO::PARAM_STR);
            $res->bindValue(2, $cpf, PDO::PARAM_STR);
            $res->bindValue(3, $senha, PDO::PARAM_STR);
            $res->bindValue(4, $email, PDO::PARAM_STR);
            $res->bindValue(5, $dataNasc, PDO::PARAM_STR);
            $res->bindValue(6, $admin, PDO::PARAM_INT);
